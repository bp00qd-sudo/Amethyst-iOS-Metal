...
payload: native dep_mg java jre assets
	...
	# MetalUniversal is intentionally embedded before the first ldid pass so
	...
	# Keep the two merged LWJGL class sets out of the root wildcard.  The
	# launcher selects one directory explicitly, so a filesystem wildcard can
	# never accidentally put 3.3.3 ahead of 3.4.1 (or vice versa).
	mkdir -p $(WORKINGDIR)/AngelAuraAmethyst.app/libs/lwjgl-3.3.3 $(WORKINGDIR)/AngelAuraAmethyst.app/libs/lwjgl-3.4.1
	cp $(SOURCEDIR)/JavaApp/build/lwjgl-3.3.3.jar $(WORKINGDIR)/AngelAuraAmethyst.app/libs/lwjgl-3.3.3/ || exit 1
	cp $(SOURCEDIR)/JavaApp/build/lwjgl-3.4.1.jar $(WORKINGDIR)/AngelAuraAmethyst.app/libs/lwjgl-3.4.1/ || exit 1
