...
int launchJVM(NSString *username, id launchTarget, int width, int height, int minVersion) {
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        ...
        // Minecraft 26.x switched to LWJGL 3.4.x.  Keep the 3.3.3 bundle for
        // older profiles, but select the 3.4.1 bindings (including SPVC) when
        // the version JSON says so.  The JSON value is more reliable than
        // guessing from a snapshot's version string.
        ...
        if ([lwjglVersionStr isKindOfClass:NSString.class] && lwjglVersionStr.length > 0) {
            ...
            NSInteger lwjglMajor = lwjglVersion.count > 0 ? [lwjglVersion[0] integerValue] : 0;
            NSInteger lwjglMinor = lwjglVersion.count > 1 ? [lwjglVersion[1] integerValue] : 0;
        ...
        NSString *inheritedVersionID = launchTarget[@"inheritsFrom"];
        NSString *minecraftVersionID = [inheritedVersionID isKindOfClass:NSString.class] && inheritedVersionID.length > 0
            ? inheritedVersionID : versionID;
        NSString *mainClass = launchTarget[@"mainClass"];
        BOOL isFabricLaunch = [mainClass isKindOfClass:NSString.class] &&
            ([mainClass containsString:@"fabricmc"] || [mainClass containsString:@"knot"]);
        // MetalUniversal currently targets Fabric/Minecraft 26.2 (Java 25).
        // Do not inject it into older Fabric instances: Fabric correctly rejects
        // a mod whose minecraft dependency is ~26.2-.
        enableMetalUniversal = [lwjglFolder isEqualToString:@"lwjgl-3.4.1"] &&
            [minecraftVersionID isKindOfClass:NSString.class] &&
            [minecraftVersionID hasPrefix:@"26.2"] && isFabricLaunch;
    ...
    // iOS, and MetalUniversal must choose the signed Frameworks loading path.
    margv[++margc] = "-Dpojav.launcher=amethyst-ios";
    ...
    if (enableMetalUniversal) {
        NSString *bundledModsPath = [NSBundle.mainBundle.bundlePath stringByAppendingPathComponent:@"bundled_mods/metallum-1.0.1.jar"];
        if ([fm fileExistsAtPath:bundledModsPath]) {
            // Metallum uses the Java 25 Foreign Function & Memory API.
            margv[++margc] = "--enable-native-access=ALL-UNNAMED";
            // Fabric's production loader accepts an explicit candidate path;
            // keep the bundled jar out of the launcher class path and do not
            // modify the user's instance/mods directory.
            margv[++margc] = [NSString stringWithFormat:@"-Dfabric.addMods=%@", bundledModsPath].UTF8String;
            // LWJGL's Spvc loader otherwise looks for its default
            // libspirv-cross name and can resolve MoltenVK's reduced symbol
            // set first.  Point it at the signed full MSL C API explicitly.
            margv[++margc] = [NSString stringWithFormat:@"-Dorg.lwjgl.spvc.libname=%@/Frameworks/libspvc.dylib", NSBundle.mainBundle.bundlePath].UTF8String;
        } else {
            NSLog(@"[JavaLauncher] MetalUniversal jar missing at %@; continuing without the mod", bundledModsPath);
        }
    }
    ...
    // The payload keeps merged LWJGL jars in versioned directories, outside
    // the root wildcard.  Select exactly one set for this Minecraft profile.
    NSString *classpath = [NSString stringWithFormat:@"%@/%@/%@.jar:%@/*:%@/%@/*",
        librariesPath, lwjglFolder, lwjglFolder, librariesPath, librariesPath, lwjglFolder];
    if (launchJar) {
