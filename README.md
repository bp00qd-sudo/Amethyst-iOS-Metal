...
#### MetalUniversal build
This fork's `Amethyst iOS Metal build` workflow produces two complete (non-slimmed) artifacts:

- `amethyst-metal-ios-ipa` for AltStore/SideStore or another re-signing tool;
- `amethyst-metal-ios-tipa` for TrollStore.

The package already contains Java 25, LWJGL 3.4.1 with `org.lwjgl.util.spvc.Spvc`, the signed `libmetallum.dylib`/`libspvc.dylib`, and the MetalUniversal mod. For a Fabric 26.2 profile the launcher passes the bundled mod through Fabric's `fabric.addMods` candidate path; no manual JAR or dylib injection is needed. Keep JIT enabled (TrollStore enables it automatically) and use the full artifact rather than a slimmed package.
